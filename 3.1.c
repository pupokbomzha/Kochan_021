...
matr* create_matrix(unsigned int n, unsigned int m);                      // Создать список
 ...
 */
matr* create_matrix(unsigned int n, unsigned int m)
{
    // Выделяем в heap (куча) память под новый список (на самом деле только под контейнер)
    matr* p_new_matrix = (matr*)malloc(sizeof(matr));
